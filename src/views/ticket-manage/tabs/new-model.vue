<template>
    <div class="showDetailBackground">
        <div class="myself-checkBox">
            <div class="myself-checkBox-tips"><span class="el-icon-warning"></span>每批次制券总数不可超过10000张！</div>
        ...
        <div class="table-scrollable">
            <b-table striped hover bordered show-empty :items="newcouponlist" :fields="couponfields">
                <template slot="nums" slot-scope="data">
                    <b-form-input type="number" @input="checkNum(data.item.makeCount,data.index)" onkeyup="this.value=this.value.replace(/^0|\D/g,'')" :readonly="hasMade || data.item.disabled" v-model="data.item.makeCount"/>
                </template>
        ...
        <b-modal id="modalPrevent"
          ...
          <!--  <h4>门店名称: {{ modelStoreName }}</h4>-->
            <div class="numsTotal">
                <span>已制券: {{ totalCard ? totalCard : 0 }}</span>
            ...
            <div class="table-scrollable controlfontsize">
                <b-table striped hover bordered show-empty :items="modellist" :fields="modelfields">
                    ...
                    <template slot="couponStatus" slot-scope="data">
                        ...
                        {{ data.item.couponStatus === 8 ? '退券中': '' }}
                        {{ data.item.couponStatus === 9 ? '已退券': '' }}
                        {{ data.item.couponStatus === -1 ? '已作废': '' }}
                    </template>
                    <template slot="couponPrice" slot-scope="data">
                    	{{ data.item.couponPrice | couponPrice}}
                    </template>
                    <template slot="empty">
                        暂无数据...
                    </template>
    ...
    export default {
        ...
        methods: {
        	checkNum(num,index){
        		...
        		if(newNum <= 10000){
        		   return 
        			...
        			this.newcouponlist[index].makeCount = null
        		}
        	},
            searchInputClick() {
                const _this = this
                ...
                if(_this.checkBoxValue) { 
                    ...
                    for(let i = 0; i< _this.tempNewcouponlist.length; i++) {
                        let judge = true
                        let judgeStr = JSON.stringify(_this.tempNewcouponlist[i])
                        for(let j = 0; j< tempArrTwo.length; j++) {
                            if(!judgeStr.includes(tempArrTwo[j])) {
                                judge = false
                            }
                        }
                        if(judge && !_this.tempNewcouponlist[i].disabled) {
                            _this.newcouponlist.push(_this.tempNewcouponlist[i])
                        }
                    }
                    // for(let i = 0; i< _this.tempNewcouponlist.length; i++) {
                    //     if(_this.tempNewcouponlist[i].storeName.startsWith(_this.checkBoxValue) && _this.tempNewcouponlist[i].disabled) {
                    //         _this.newcouponlist.push(_this.tempNewcouponlist[i])
                    //     }
                    // }
                    for(let i = 0; i< _this.tempNewcouponlist.length; i++) {
                    ...
                    for(let i = 0; i< _this.tempNewcouponlist.length; i++) {
                        if(!_this.tempNewcouponlist[i].disabled) {
                            _this.newcouponlist.push(_this.tempNewcouponlist[i])
                        }
                    }
                    for(let i = 0; i< _this.tempNewcouponlist.length; i++) {
            ...
            makeCoupon() {
                const _this = this
                let tempArr = []
                _this.tempNewcouponlist.forEach(element => {
                ...
                let option = {
                    ...
                    hasTimestamp: this.showObj.hasTimestamp,
                    couponPrefix: this.showObj.couponPrefix,
                    couponMakeDetailInfoList: tempArr
                }
                for(let i = 0; i< option.couponMakeDetailInfoList.length; i++) {
                    if((option.couponMakeDetailInfoList[i].makeCount-0) < 0) {
                        Message({
                            type: 'warning',
                            message: '制券数量不能为负数'
                        })
                ...
                let judgeNums = 0
                for(let i = 0; i<option.couponMakeDetailInfoList.length; i++) {
                    if(option.couponMakeDetailInfoList[i].makeCount === undefined) {
                        option.couponMakeDetailInfoList[i].makeCount = 0
                    }
                    judgeNums+=(option.couponMakeDetailInfoList[i].makeCount - 0)
                }
                if(judgeNums>10000) {
                    Message({
                        type: 'warning',
                        message: '批次制券总数量不能超过10000'
                    })
                    return;
                }
                let judgeEmpty = true
                ...
                if(judgeEmpty) {
                    Message({
                        type: 'warning',
                        message: '门店制券数量全部为空或为零，不允许制券'
                    })
                    return
                }
                for(let i = 0; i< option.couponMakeDetailInfoList.length; i++) {
                    if(Math.floor(option.couponMakeDetailInfoList[i].makeCount - 0) === (option.couponMakeDetailInfoList[i].makeCount - 0) ) {
                ...
                let param = {
                    serviceCode: config.cardRelative.detailSeq_,
                    getNums: option.couponMakeDetailInfoList.length
                }
                ...
                api.ordinalInfo.getSequenceList(param, res => {
                    if(res.data.code === 'success') {
                        for(let i = 0; i<res.data.obj.length; i++) {
                            option.couponMakeDetailInfoList[i].couponMakeDetailCode = res.data.obj[i]
                            option.couponMakeDetailInfoList[i].couponMakeOrderNo = _this.couponMakeOrderNo
                            option.couponMakeDetailInfoList[i].couponTemplateCode = _this.couponTemplateCode
                        }
                        api.ticketManage.makeCouponPici(option, res => {
                            if(res.data.code === 'success') {
            ...
            clear() {
            	debugger
                const _this = this
                _this.newcouponlist.forEach(element => {
                    element.makeCount = 0
                })
            },
            // // 查询总量
            queryTotal() {
                ...
                api.ticketManage.queryTableTotal(option, res => {
                    if(res.data.code === 'success') {
                        ...
                        this.modalTitle = `门店名称:${this.modelStoreName}`
                    }
                })
            },
            // 查询
            ...
            giveDataToNew() {
                ...
                for(let i = 0; i< _this.storeList.length; i++) {
                    if(!_this.storeList[i].disabled) {
                        ...
                        _this.tempNewcouponlist.push(obj)
                    }
                }
                for(let i = 0; i< _this.storeList.length; i++) {
                    if(_this.storeList[i].disabled) {
                ...
                _this.newcouponlist = _this.tempNewcouponlist
            },
            // 获取区域   并查询门店是否制卡
            getShouldAreacoupon() {
              ...
              /*  if(this.useNatureTypeCode == 'SpecialShop'){
                	option.storeCode = 
                }*/
                api.ticketManage.queryShouldAreaCoupon(option, res => {
                    if(res.data.code === 'success') {
                        ...
                        api.ticketManage.queryStoreStatus(option, res => {
                            if(res.data.code === 'success') {
                                for(let i = 0; i< _this.storeList.length; i++) {
        ...
        watch: {
            tempNewcouponlist: {
                handler: function() {
                    ...
                    this.tempNewcouponlist.forEach(element => {
                        if(element.makeCount) {
