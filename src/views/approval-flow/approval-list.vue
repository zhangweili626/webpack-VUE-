		...
		<b-modal hide-footer title="待办事项列表" ref="myModel" size="lg" id="emp-insert2" @show="reload">
			...
			<b-table striped hover bordered :items="list" :fields="field" show-empty empty-text="暂无数据" class="mt-3">
				<template slot="orderNo" slot-scope="data" >
					<div @click="hideModal">
						<!--<span v-if="islivecs == true">
							<router-link  :to="{name:'approval-flow-details',query:{
		                    	orderNo:data.item.orderNo,
		                    	billtype:data.item.orderTypeCode,
		                    	orderWfCode:data.item.orderWfCode,
		                    	onlySee:true,
		                    }}">
		                      {{data.item.orderNo}}
		                  	</router-link>
...
export default{
	...
	mounted(){
		...
		this.getList();
		this.getNumSlist();
		this.getOrigin = common.getUploadUrl();
	},
	created(){
