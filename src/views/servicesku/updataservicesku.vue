<template>
    <b-modal id="updata" ref="updata" title="编辑商品适配" size="lg" @ok="handleOk" ok-title="确定" @shown="showmodal" cancel-title="取消">
        <div class="row">
            <div class="col-md-6">
...
export default {
    ...
    methods: {
        ...
        handleOk(evt) {
            ...
            api.serviceSku.editServiceSkuRelationInfo([_this.params],(msg)=>{
                if(msg.data.message == 'success'){
                        _this.$emit('service-ok',_this.params.serviceCode)     
                        _this.params.skuCategoryName = null
                        _this.params.skuCategoryCode = null
                        _this.$refs.skusearch.setValue()              
                        _this.params.skuCode = null  
                        ...
                        _this.skuSelectParams.originalCode = null   
                        Message({
                        type: 'success',
                        message: "操作成功！"
                    });
        ...
        showmodal() {
            ...
            _this.brandDatalist = []          
            _this.skuDatalist = []      
            if(_this.relationCode == null){
                _this.$refs.updata.hide()      
            }
        },
         //品牌下拉框清除事件
        brandClearValue(){
        ...
        skuQuerySelect(data) {
            ...
            _this.skuSelectParams.skuName = data;
    ...
    watch:{
        relationCode:function(value){
            const _this = this
            api.serviceSku.queryServiceSkuRelationInfoForPageByVo({
                relationCode:value
            },(msg)=>{
                if(msg.data.message == 'success'){
                    _this.params = msg.data.obj.list[0]  
                    if(_this.params.needQty == 0){
                        _this.params.needQty = null
                    }   
                    _this.$refs.brandsearch.setValue(_this.params.skuBrandName)  
                    _this.$refs.skusearch.setValue(_this.params.skuName)                                    
                }
            })
        }
    }
