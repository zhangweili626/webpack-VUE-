                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-lg-6">
                        <b-form-fieldset horizontal label="前缀*" label-text-align="right" :label-cols="4">
                            <b-form-input  placeholder="请填写字母区分大小写" :disabled="disabled" :state="state.couponPrefix" v-model="param.couponPrefix" @input="couponPrefixChange()" />
                        </b-form-fieldset>
                    </div>
                    <div class="col-md-6 col-lg-6">
                ...
                <div class="staticTime">
                    <b-form-fieldset horizontal label="有效期类型*" label-text-align="right" :label-cols="2">
                        <b-container fluid>
                            <b-row class="my-1">
                                <b-col sm="5">
                                    <input :disabled="true" @change="couponExpireTypeChange()" :class="{'inp-invalid' : state.couponExpireType}" type="radio" name="time" value="0"  v-model="param.couponExpireType">&nbsp;&nbsp;固定时长
                                </b-col>
                                <b-col sm="7">
                                    <div class="row">
                                        <div class="col-md-10">
                            ...
                            <b-row class="my-1">
                                ...
                                <b-col sm="7">
                                    <el-date-picker :disabled="(param.couponExpireType==0)||disabled" :class="{'inp-invalid' : state.expireDate}" v-model="param.expireDate" type="daterange" placeholder="选择日期" :picker-options="pickerOptions1">
                                    </el-date-picker>
                                </b-col>
                            </b-row>
                        </b-container>
        ...
        <div class="row ml-2 mr-2 mb-2">
            <div class="col-md-12">
                <div class="pull-right">
                    ...
                    <b-button :disabled="!detailShow" v-if="param.couponTemplateStatus == 2 && !publishFlag" size="sm" variant="primary" @click="recallpublish">撤销发布</b-button>
                    <b-button :disabled="!detailShow" v-if="param.couponTemplateStatus == 0 && !publishFlag" size="sm" variant="primary" @click="publish">发布</b-button>
                </div>
            </div>
        </div>
            ...
            <b-tabs>
                <b-tab title="内容">
                    <div class="row mb-2">
                        <div class="col-md-12">
                            <div class="pull-left">
                                <b-button v-if="!disabled" size="sm" variant="danger" @click="deleteDetail">删除</b-button>
                                <b-button v-if="!disabled" size="sm" variant="success" :disabled="(!!containerlist.length)" @click="addContainer">添加</b-button>
                            </div>
                            <!-- <div class="pull-right"> -->
...
export default {
    ...
    methods: {
            ...
            api.ticketTemlate.releaseCouponTemplateInfo(param, (msg) => {
                if (msg.data.message == 'success') {
                    ...
                    api.ticketManage.queryMainInfoCard({
                        couponTemplateCode: _this.$route.query.couponTemplateCode                   
                    },(msg)=>{
                        if(msg.data.message == 'success'){
                            ...
                            _this.param = {
                                ...
                                expireDate:[data.startDate||"",data.endDate||""],
        ...
        preserve() {
            ...
            if (!(_this.param.expireDate.length == 0)) {
                _this.param.startDate = common.formattingTime([_this.param.expireDate[0], _this.param.expireDate[1]]).startTime
                _this.param.endDate = common.formattingTime([_this.param.expireDate[0], _this.param.expireDate[1]]).endTime
            }
    ...
    watch:{
        'param.couponTemplateStatus':function(){
            const _this = this;
            if(_this.param.couponTemplateStatus==2){
                _this.disabled = true
            }
        },
    },
    created() {
        ...
        if(Object.prototype.toString.call(_this.$route.query.disabled ) === "[object String]"){
            this.disabled = _this.$route.query.disabled == 'true' ? true : false      
            this.publishFlag = this.disabled              
        }else{
            this.disabled = _this.$route.query.disabled   
            this.publishFlag = this.disabled              
        }
        api.ticketManage.queryMainInfoCard({
            couponTemplateCode: _this.$route.query.couponTemplateCode                   
            ...
            if(msg.data.message == 'success'){
                ...
                _this.detailShow = true
                _this.frontuploadParams.relationCode = _this.param.couponTemplateCode
                _this.reverseuploadParams.relationCode = _this.param.couponTemplateCode
                _this.brand1uploadParams.relationCode = _this.param.couponTemplateCode
                _this.brand2uploadParams.relationCode = _this.param.couponTemplateCode
                _this.brand3uploadParams.relationCode = _this.param.couponTemplateCode
                let userInfo = sessionStorage.getItem("userInfo");  
                if(userInfo){
                    _this.param.empCnName = JSON.parse(userInfo).empVo.empCnName     
                    _this.param.makeOrgCode = JSON.parse(userInfo).empVo.orgCode     
        ...
        api.accessory.list({
                relationCode: _this.$route.query.couponTemplateCode
        }, function (result) {
