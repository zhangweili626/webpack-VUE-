<template lang="html">
  <div class="scope">
    ...
    <div>
        <div class="row mb-2">
            <div class="col-md-12">
                <div class="pull-left"><button v-if="!disabled" @click="addScopeBtn" type="button" class="btn btn-success btn-sm mb-2">新增</button>
                <button v-if="!disabled" @click="deletScopeBtn" type="button" class="btn btn-danger btn-sm mb-2">删除</button>
            </div>
                <div class="pull-right">
                    <b-button v-if="!disabled" size="sm" variant="success" @click="submit">保存</b-button>
                </div>
            </div>
        </div>
        ...
        <tbody>
            <tr v-for="val in myTablist">
                <td>
                    <input type="radio" :value="val" name="radio"  v-model='index'>
                </td>                
                <td v-for="(item, index) in fields" :key="index">
                    <span v-if="!item.edit">{{val[item.name]}}</span>
                    <b-form-input :disabled="disabled" style="width:200px;" type="number" v-if="item.edit" v-model="val[item.name]"/>
                </td>
            </tr>
        <tr v-if="myTablist.length==0">
...
import salesArea from 'components/iris-scope/area'
import chinaarea from 'components/iris-scope/chinaarea'
import store from 'components/iris-scope/store'
...
export default {
  ...
  methods: {
    submit(){
        const _this = this
        for (let index = 0; index < this.myTablist.length; index++) {
            const element = this.myTablist[index];
            let patrn=/^([1-9]\d*|0)(\.\d*[1-9])?$/;
            if(!patrn.exec(element.couponPrice)){
                Message({
                    type: 'warning',
                    message: '请填写正确的售价，售价不可为负数！'
                })
                return;
            }
        }
        api.ticketTemlate.editCouponTemplatePriceRangeInfo(this.myTablist, (msg) => {
