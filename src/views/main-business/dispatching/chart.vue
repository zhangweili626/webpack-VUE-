    ...
    <div class="dc-left pull-left">
        <div class="left-item" v-for="(item, i) in leftList" :key="i" :style="{width: leftWidth + 'px'}">
            <div v-if="item.id === 'select'">
                <b-form-select v-model="selectWork" @input="selectWorkInput" :options="workType" ref="select">
                </b-form-select>
            </div>
            {{item.name}}
        </div>
...
import Model from '../resource-utilization/tc-detail/model'
...
export default {
    ...
    methods: {
        selectWorkInput(){
            this.$emit('selectWork',this.selectWork)
        },
        redLineLeft() {
    ...
    watch: {
        ...
        serviceCode:function(){
            this.selectWork =  this.serviceCode || this.workType[0].value
        }
    },
    destoryed() {
