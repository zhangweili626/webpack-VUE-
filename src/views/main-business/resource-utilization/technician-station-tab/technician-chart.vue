...
<div class="tc-container clearfix">
    ...
    <div class="tc-content pull-left" :style="{marginLeft: leftWidth + 'px'}">
        ...
        <div class="line-red" :style="{height: lineHeight + 'px', left: redLineLeft() + 'px'}" 
           ref="redLine" v-if="showRedLine"></div> 
    </div>
</div>
</template>
<script>
 ...
 * @param defaultStartTime 默认开始时间 ( 支持到分钟 ) default '07:30'
 * @param defaultEndTime 默认结束时间 ( 支持到分钟 ) default '17:10'
 * @param leftWidth 左侧宽度 ( 支持最小宽度50 ) default 120
 * @param lineWidth 辅助线宽度 ( 不需要可以传0 ) default 1
...
export default {
    ...
    computed: {
        ...
        showRedLine() {
            let currentTime = new Date(formatDate(new Date())).getTime()
            let propTime = new Date(this.selectDate).getTime()
            // 当前时间大于刻度结束时间, 红线消失
            let defaultEndMinutes = new Date(`${formatDate(new Date())} ${this.defaultEndTime}`).getTime()
            let currentMinutes = new Date().getTime()
            if(currentMinutes > defaultEndMinutes) {
                return false
            }
            return currentTime == propTime 
        }
    },
    mounted() {
        // 后续delete, 需要监听数据变化而更新视图
        ...
        this.t = setInterval(() => {
            try {
                ...
                // 每分钟计算可视范围, 个人觉得这里不能限制死, 留给用户操作空间太少 ( 定时时间长短决定用户体验 )
                this.scrollView()
            }catch (e) {}  
        }, 30000)
    },
    methods: {
        ...
        listChange() {
            ...
            this.list.forEach((item, j) => {
                ...
                for(let i = 0; i < item.duration.length; i ++) {
                    let startArr = item.duration[i].startTime&&item.duration[i].startTime.split(':')
                    let endArr = item.duration[i].endTime&&item.duration[i].endTime.split(':')
                    let startMinutes = parseInt(startArr&&startArr[0]) * 60 + parseInt(startArr&&startArr[1])
                    let endMinutes = parseInt(endArr&&endArr[0]) * 60 + parseInt(endArr&&endArr[1])
                    // 这里有个逆向思维, 必须是当前开始时间 - 上一个结束时间 得出下一个时间段的marginleft值, 且不能用变量保存
                    let obj = {
                        dispatchDetailCode: item.duration[i].dispatchDetailCode,
                        width: (endMinutes - startMinutes) * this.minuteWidth,
                        marginLeft: i === 0 ? (startMinutes - this.getTotalMinutes(this.defaultStartTime)) * this.minuteWidth + this.tempWidth / 2
                                            : (startMinutes - (parseInt(item.duration[i - 1].endTime.split(':')[0]) * 60 + parseInt(item.duration[i - 1].endTime.split(':')[1]))) * this.minuteWidth
                    }
