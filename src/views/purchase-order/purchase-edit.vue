    ...
    <div class="car-template card">
        ...
        <div class="row">
            <div class="col-md-4">
                <b-form-fieldset horizontal label="销售区域*" :label-cols="4" class="text-right">
                    <AreaTree :readonly='true' :error="AreaTreeError" @select-change="selectedfun" ref="area" :storeAll='true'></AreaTree>
                </b-form-fieldset>
            </div>
            <div class="col-md-4">
                <b-form-fieldset horizontal label="仓库名称*" :label-cols="4" class="text-right">
                    <b-form-select  :disabled="fieldDisabled" :state="state.whCode" v-model="params.whCode" :options="entreList"></b-form-select>
                </b-form-fieldset>
            </div>
            ...
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset horizontal label="供应商类型*" :label-cols="4" class="text-right">
                    <b-form-select  :disabled="fieldDisabled" :state="state.supplierTypeCode" @input="supplierTypeChange" v-model="params.supplierTypeCode" :options="SupplierType"></b-form-select>                        
                </b-form-fieldset> 
            </div>
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset  horizontal label="供应商*" :label-cols="4" class="text-right">
                    ...
                    <b-form-select :disabled="fieldDisabled" :state="state.supplierCode" v-model="params.supplierCode" @input="supplierCodeInput" :options="supplierList"></b-form-select>
                </b-form-fieldset> 
            </div>
            <div class="col-md-4 col-lg-4">
            ...
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset horizontal label="配送类型*" :label-cols="4" class="text-right">
                    <b-form-select :disabled="fieldDisabled"  :state="state.deliveryTypeCode" v-model="params.deliveryTypeCode" :options="DispatchingType"></b-form-select>                        
                </b-form-fieldset> 
            </div>
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset horizontal label="承运商" :label-cols="4" class="text-right">
                    <b-form-input :disabled="fieldDisabled" v-model="params.commonCarrierName"></b-form-input>                        
                </b-form-fieldset> 
            </div>
            <div class="col-md-4 col-lg-4">
            ...
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset horizontal label="运费" :label-cols="4" class="text-right">
                    <b-form-input :state="state.totalFreightFee" :disabled="fieldDisabled" type="number" v-model="params.totalFreightFee"></b-form-input>                        
                </b-form-fieldset> 
            </div>
            <div class="col-md-4 col-lg-4">
                <b-form-fieldset horizontal label="备注" :label-cols="4" class="text-right">
                    <b-form-input :disabled="fieldDisabled" v-model="params.remark"></b-form-input>                        
                </b-form-fieldset> 
            </div>
        </div>
        <div class="row ml-2 mr-2">
            <div class="col-md-12">
                <div class="pull-right">
                    ...
                    <b-button v-if="params.orderStatus != 1" v-show="!disabled" size="sm" variant="primary" @click="preserve()">保存</b-button>                        
                    <b-button  v-if="params.orderStatus == 0" :disabled="!flag" size="sm" variant="primary" @click="submit()">提交</b-button>                        
                    <b-button  v-if="!disabled" size="sm" variant="danger" @click="cancelOrder()">作废</b-button>                        
                </div>
            </div>
        </div>
        </b-card>        
        <b-card header="明细">
            <div class="row">
                <div class="col-md-12">
                    <div class="pull-left">
                        <b-button v-show="params.orderStatus != 1" v-if="!disabled" size="sm" variant="danger" @click="cancel()">删除</b-button>                        
                        <b-button v-show="params.orderStatus != 1" v-if="!disabled" size="sm" variant="primary" @click="addDetail">增加</b-button>                        
                    </div>
                </div>
            </div>
                ...
                <el-table
                    ...
                    :width="getColumnW('选择', '')">
                        <template slot-scope="scope">
                            <input  :disabled="fieldDisabled" type="checkbox" name="radio"  v-model="selCont" :value="scope.row"/>                                
                        </template>	
                    </el-table-column>
                    <el-table-column
                    prop="skuCode"
                    ...
                    <template slot-scope="scope">
                        <!-- <b-form-input :disabled="fieldDisabled" type="number" min="0" @blur="requirementNum(scope.row)" v-model="scope.row.skuNums"></b-form-input>                         -->
                        <input class="form-control" :disabled="fieldDisabled" type="number" min="0" @blur="requirementNum(scope.row)" v-model="scope.row.skuNums"/>                                
                    </template>	
                    </el-table-column>
                    <el-table-column
                    prop="purchaseFee"
                    ...
                    <template slot-scope="scope">
                        ...
                        <div v-if="scope.row.id">
                            <input :disabled="fieldDisabled"  class="form-control" v-if="!scope.row.contractDetailCode" @blur="purchaseFeeBlur(scope.row)" type="number" min="0" v-model="scope.row.purchaseFee"/>  
                            <span v-if="scope.row.contractDetailCode">{{scope.row.purchaseFee.toFixed(2)}}</span>                              
                        </div>
                    </template>	
                    </el-table-column>
                    <el-table-column
                    ...
                    <template slot-scope="scope">
                        <span>{{ scope.row.Tax = ((scope.row.IncludingTax / (1+(scope.row.purchaseRate||0))*scope.row.purchaseRate) || 0).toFixed(2)}}</span>                              
                    </template>
                    </el-table-column>
                    <el-table-column
                    prop="exclusiveOfTax"
        ...
        <irisModal ref="irisModal" @goBack="goBack" :tipsTitle="'确认变更么？'" :warnigTitle="'供应商发生改变，单据数据需要重新生成'" @sureEvent="sureEvent"></irisModal>
...
<script>
    ...
    export default {
        ...
        methods: {
            goBack(){
                const _this = this;  
                _this.params.supplierTypeCode = _this.supplierTypeCode              
            ...
            cancelOrder(){
                const _this = this;                
                api.purchaseOrder.cancelPurchaseOrderInfo({
                        ...
                        _this.preserve(function(){
                            api.purchaseOrder.submitPurchaseOrderInfo({
                                ...
                                if(res.data.message == 'success'){
                                    ...
                                    _this.params.orderStatus = 1
                                    _this.fieldDisabled = true
                                }                    
                            })
                        })
            ...
            requirementNum(obj){
                const _this = this
                console.log(parseFloat(obj.skuNums)%1)
            ...
            preserve(callback){
                ...
                if((_this.params.supplierTypeCode == 'REFD2000181') || (_this.params.supplierTypeCode == 'REFD2000182')){
                    ...
                    for (let index = 0; index < _this.tableList.length; index++) {
                        ...
                        if(parseFloat(element.skuNums) <= 0){
                            console.log(parseFloat(element.skuNums))
                            Message({ 
                                type: 'warning', 
                                message: '请填写正确的需求数量！'
                            });
                ...
                api.purchaseOrder.updatePurchaseOrderInfoAndPoSkuInfo(_this.params,(msg)=>{
        ...
        created(){
            ...
            api.purchaseOrder.getPurchaseOrderInfoByCode({
                orderNo:_this.$route.query.orderNo
            },(msg)=>{
                if (msg.data.code === "success") {
                    let obj = msg.data.obj
                    _this.$refs.area.setselect({text:obj.storeName,value:obj.storeCode})
                    _this.selectParams.storeCodes = [obj.storeCode]
                    _this.params = {
                        whCode:obj.whCode,
                        storeCode:obj.storeCode,
                        purchaseTypeCode:obj.purchaseTypeCode,
                        supplierTypeCode:obj.supplierTypeCode,
                        supplierCode:obj.supplierCode,
                        deliveryTypeCode:obj.deliveryTypeCode,
                        commonCarrierName:obj.commonCarrierName,
                        estimatedArrivalDate:obj.estimatedArrivalDate,
                        deadLineTime:obj.deadLineTime,
                        totalNums:obj.totalNums,
                        totalMoney:obj.totalMoney.toFixed(2),
                        totalFreightFee:obj.totalFreightFee,
                        remark:obj.remark,
                        orderNo:obj.orderNo,
                        id:obj.id,
                        orderType:1,
                        orderStatus:obj.orderStatus,
                        createTimeStr:obj.createTimeStr,
                        purchaseTypeCode:obj.purchaseTypeCode,
                    }
                    _this.supplierTypeCode = obj.supplierTypeCode
                    _this.supplierCode = obj.supplierCode
                    _this.tableList = obj.poSkuDetailInfoVos
                    if(obj.orderStatus == 1){
                        _this.fieldDisabled = true
                    }
                    _this.selectParams.supplierType = obj.supplierTypeCode
                    api.purchaseOrder.querySupplierInfoNoPageBySupplierInfo(_this.selectParams, (res) => {
                        if (res.data.code === "success") {
                            let obj = res.data.obj;
                            for (let index = 0; index < obj.length; index++) {
