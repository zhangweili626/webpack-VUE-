        ...
        <b-card header="主表明细">
            <b-form id='addQuery' disabled="">
	            ...
	            <div class="row">
	                ...
	                <!--<div class="col-md-4 col-lg-4">
	                    <input type="number" v-model="data.item.outNums" min="1"/>
			                ...
			                <b-table striped hover bordered show-empty :items="tablelist1" :fields="fields_1" v-if="addQuery.billTypeSelected=='1'">
			                    ...
			                    <template slot="skuCode" slot-scope="data">
			                     ...
			                     <template slot="salePrice" slot-scope="data">
			                      	<b-form-input type="text" style="width:100px" v-if="data.item.raiseType =='1'" v-model="data.item.salePrice" :disabled="subInputDisabled"/>
			                    	<span v-if="data.item.raiseType == '0'">{{getSalePrice(data)}}</span>
			                    	<span v-if="data.item.raiseType == '2'">{{data.item.salePriceView}}</span>
			                    </template>
			                    ...
			                    <template slot="raiseValue" slot-scope="data">
			                    	<b-form-input type="text" :disabled="subInputDisabled" style="width:100px" v-if="data.item.raiseType =='0'" v-model="data.item.raiseValue"/>
			                    </template>
			                    <template slot="raiseType" slot-scope="data">
			                    	<b-form-select :options="addPriceType"  :disabled="subInputDisabled" v-model="data.item.raiseType" style="width: 100px;" @input="getSalePrice(data)"></b-form-select>
			                    </template>
			                    <template slot="taxRateCode" slot-scope="data">
			                    	<b-form-select :options="rateValue" :disabled="subInputDisabled" v-model="data.item.taxRateCode" style="width: 100px;"></b-form-select>
			                    </template>
			                    <template slot="outNums" slot-scope="data">
			                    	<b-form-input  type="number" v-model="data.item.outNums" min="1" @change="countResidue(data.value,data.index)" :disabled="subInputDisabled"/>
			                    </template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                availableNums:[],
                deleList:[],
                addPriceType:[//0：加价系数、1：固定金额、2：引用销售单价
                	{
                		value:0,
                		text:'加价系数'
                	},{
                ...
                rateValue:[],
                rateValueSelected:null,
                PriceTypeSelected:null,
                raiseValueSelected:'',
                deletIsOpen:false,
                getIndex:null,
                salePrice:null,
        ...
        methods: {
            ...
            getSalePrice(data){
            	if(data.item.raiseType == '0'){
            		let index = this.rateValue.findIndex(v => v.value == data.item.taxRateCode )
	            	if(data.item.taxRateCode != ''){
	            		let a = JSON.parse(this.toPoint(this.rateValue[index].text))
	        			return (data.item.stockCost*(1+ a)*data.item.raiseValue).toFixed(2);
	            	}
            	}
            },
            // 判断编辑与添加页面的方法
            ...
            submitTable(){
                ...
                api.allotout.submitTable(option, (res) => {
                    if(res.data.code === 'success') {
	                	...
	                	//this.$refs.myapproval.getInfo(this.$route.params.transferOutOrderNo)
	                	Message({
	       					message:'提交成功',
	       					type:'success'
	       				})
            ...
            RateValue(){
                    ...
                    api.ref.getDataDictionarys(options, (res) => {
                    	let arry = res.data.obj.referenceDetailInfos;
	                    if(res.data.code === 'success'){
	                    	arry.forEach(function(item,i){
	                    		_this.rateValue.push({
	                    			text:Number(item.refDetailName)*100+"%",
            ...
            getQuery(){
                ...
                api.allotout.selectQuery(option, (res) => {
                    if(res.data.code === 'success') {
                       ...
                       _this.addQuery.id=obj.id;
                       _this.addQuery.statussave=obj.orderStatus;
                       _this.addQuery.odd = obj.transferOutOrderNo;
                       _this.addQuery.billTypeSelected = obj.transferSkuType;
                       _this.addQuery.billTypeSubSelected = obj.transferType;
                       _this.addQuery.outNum = obj.totalNums;
                       _this.addQuery.inNum = obj.realNums;
                       _this.addQuery.auditState=obj.wfStatus;
                       _this.addQuery.supplyin= obj.inWhCode;
                       _this.addQuery.supplyout= obj.outWhCode;
                       _this.addQuery.inStoreName= obj.inStoreName;
                       _this.addQuery.outStoreName= obj.outStoreName;
                       _this.addQuery.money= obj.totalMoney.toFixed(2);
                       _this.addQuery.realMoney= obj.realMoney.toFixed(2);
                       _this.addQuery.startTime= obj.confirmOperatorTime;
                       _this.addQuery.endTime= obj.estimatedArrivalDate;
                       _this.addQuery.freight= obj.totalFreightFee;
                       _this.addQuery.status= _this.billuStatus(obj.orderStatus);
                       this.realNums = obj.realNums;
                       if(obj.transferSaleOutSkuDetailInfoVoList == null){
                       		_this.tablelist1 = []
                       }else{
                       		_this.tablelist1 = obj.transferSaleOutSkuDetailInfoVoList;
                       }
                      _this.btnStatus(obj.orderStatus);
                      _this.$refs.inareaqueryshop.setselect({text:obj.inStoreName,value:obj.inStoreCode})
                      _this.$refs.outareaqueryshop.setselect({text:obj.outStoreName,value:obj.outStoreCode})
                      // _this.addQuery.billTypeSubSelected = obj.transferType;
                    }
                })
            },
	       ...
	       /* querySelect(data) {
	        ...
	        //查询价格接口
	        querySkuPrice(option){
	        	api.allotout.querySkuPrice(option,function(res){
	        ...
	        itemValue(value) {
	            ...
	            api.allotout.querySkuPrice(options,function(res){
	        		if(res.data.code == 'success'){
	        			 _this.tablelist1[_this.index].stockCost = res.data.obj.stockCost;
	        			 _this.tablelist1[_this.index].salePriceView = res.data.obj.salePriceView;
	        		}
	        	})
								...
								if(res) {
									...
									for(let i = 0; i< _this.tablelist1.length; i++) {
										if(_this.tablelist1[i].skuCode === option.skuCode) {
											...
											_this.tablelist1[i].outNums=obj.outNums;
											_this.tablelist1[i].originalCode=obj.originalCode;
											_this.tablelist1[i].externalCode=obj.externalCode;
											_this.tablelist1[i].brandName=obj.brandName;
											_this.tablelist1[i].skuStandards=obj.skuStandards;
											_this.tablelist1[i].skuModel=obj.skuModel;
											_this.tablelist1[i].purchaseUnitName=obj.purchaseUnitName;
											_this.tablelist1[i].skuTempCode=obj.tempSkuCode;
											_this.tablelist1[i].skuName=obj.skuName;
											...
											_this.tablelist1[i].barCode = obj.barCode;
										}
									}
				         	}});
			            	}
        ...
        getUser(){
         ...
         console.log(this.addQuery.user)
            ...
            publicDelet(){
        	   ...
        	   api.allotout.editSaleOutDetail(this.deleList, (res) => {
                    if(res.data.code === 'success') {
                    	this.tablelist1 =[];
                    	Message({
	       					message:'删除成功',
           ...
           editDetail(){
           	    const _href=location.href.split('/');
           	    console.log(this.tablelist1[0].raiseType);
           		...
           		for(let i = 0; i < _this.tablelist1.length; i++) {
           					if(_this.tablelist1[i].skuCode == '') {
                                ...
                                return;
   					  		...
   					  		if( _this.tablelist1[i].raiseType == null) {
                                Message({
			       					message:'请选择加价类型',
			       					type:'warning'
			       				})
                                return;
   					  		...
   					  		if( _this.tablelist1[i].taxRateCode =='') {
                                Message({
			       					message:'请选择加价税率',
			       					type:'warning'
			       				})
                                return;
   					  		...
   					  		if( _this.tablelist1[i].raiseType == 1) {
                               if(_this.tablelist1[i].salePrice =='') {
                                Message({
			       					message:'请填写销售价',
			       					type:'warning'
			       				})
                                return;
   					  		...
   					  		if( _this.tablelist1[i].raiseType == 0) {
                               if(_this.tablelist1[i].raiseValue =='') {
                                Message({
			       					message:'请填写加价值',
			       					type:'warning'
			       				})
           	    ...
           	    option.forEach(function(item){
           			...
           			item.empToken=JSON.parse(common.getSession('userInfo')).empToken;
           		});
                api.allotout.editSaleOutDetail(option, (res) => {
                    if(res.data.code === 'success') {
                    	_this.getQuery();
                    	_this.deleList = [];
                    	Message({
          ...
          deleItem(){
            	...
            	if(_this.selected == ''&& this.tablelist1[this.getIndex].skuName != ''){
            		...
            		api.allotout.selectQuery(option, (res) => {
	                    if(res.data.code === 'success') {
	                      let arry = res.data.obj.transferSaleOutSkuDetailInfoVoList;
	                      if(arry == null){
	                      	...
	                      	return 
			                      ...
			                      if(index != -1){
			                      	   ...
			                      	   api.allotout.editSaleOutDetail(_this.deleList, (res) => {
						                    if(res.data.code === 'success') {
						                    	_this.getQuery();
						                    	Message({
							       					message:'删除成功',
