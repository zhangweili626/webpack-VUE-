    ...
    <div class="row" style="margin: 0 20px;">
        ...
        <div class="col-md-4">
            <b-form-fieldset horizontal label="开票金额*" :label-cols="4" class="text-right">
                <b-form-input type="number" ref="inputInvoiceAmountDom" v-model.trim="info.invoiceAmount" @change="invoiceAmountOnKeyUp" @input="inputInvoiceAmount" :disabled='invoiceAmountDisabled' :state="inp.invoiceAmount"/>
            </b-form-fieldset>
        </div>
        <div class="col-md-4">
...
export default {
    ...
    methods: {
        invoiceAmountOnKeyUp(){
            if(this.info.invoiceAmount < 0){
                Message({
                    type: "warning",
                    message: "开票金额不可以是负数，或者为0"
                });
                this.info.invoiceAmount = '';
                this.countPrice(0, this.info.invoiceRate)
            }
        },
        inputInvoiceAmount() {
