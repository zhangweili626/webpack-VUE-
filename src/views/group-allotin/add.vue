    ...
    <div>
        ...
        <b-card header="入库明细">
            <div v-if="canSaveDetail" class="table-scrollable">
                <b-table striped hover bordered show-empty :items="tempData.transferSaleOutSkuDetailInfoVoList" :fields="fields">
                    ...
                    <template slot="inStockCost" slot-scope="data">
                        {{ data.item.inStockCost ? (data.item.inStockCost - 0).toFixed(2) : '' }}
                    </template>
                    <template slot="taxRate" slot-scope="data">
                        {{ (data.item.taxRate ? data.item.taxRate * 100 : '') + '%' }}
                    </template>
            ...
            <div v-if="justShowDetail" class="table-scrollable">
                <b-table striped hover bordered show-empty :items="tempData.transferSaleInSkuDetailInfoVoList" :fields="fieldsShow">
                    <template slot="skuCode" slot-scope="data">
                        {{ data.item.skuCode }}
                    </template>
                    <template slot="skuName" slot-scope="data">
    ...
    export default {
        ...
        data() {
            return {
                ...
                allotinTips: '是否确认入库',
                tempData: {
                    transferSaleInSkuDetailInfoVoList: []
                },
                fields: {
                    skuCode: {
                        label: 'SKU编码'
        ...
        methods: {
                ...
                setTimeout(() => {
                    ...
                    api.skuPlace.queryWarehouseLocationInfo(option, res => {
                        if(res.data.code === 'success') {
                            ...
                            // arg.whLocationCodelist = tempArr
                            this.$set(_this.tempData.transferSaleOutSkuDetailInfoVoList[data.index], 'whLocationCodelist', tempArr)                            
                            _this.tempData.transferSaleOutSkuDetailInfoVoList = JSON.parse(JSON.stringify(_this.tempData.transferSaleOutSkuDetailInfoVoList))
                        }
                    })
                },10)
            },
            ...
            instorage() {
                ...
                _this.tempData.transferSaleOutSkuDetailInfoVoList.forEach(element => {
                ...
                _this.tempData.transferSaleInSkuDetailInfoVoList = _this.tempData.transferSaleOutSkuDetailInfoVoList ? _this.tempData.transferSaleOutSkuDetailInfoVoList : _this.tempData.transferSaleInSkuDetailInfoVoList
                _this.tempData.transferType = 1
                _this.tempData.transferSkuType = 1
                delete _this.tempData.transferSaleOutSkuDetailInfoVoList
                for(let i = 0; i< _this.tempData.transferSaleInSkuDetailInfoVoList.length; i++) {
                    if(_this.tempData.transferSaleInSkuDetailInfoVoList[i].inNums - _this.tempData.transferSaleInSkuDetailInfoVoList[i].remainNums > 0) {
                        Message({
                            type: 'warning',
                            message: '商品入库数量数量不合理'
                        })
                ...
                let judge = false
                let tempArr = _this.tempData.transferSaleInSkuDetailInfoVoList
            ...
            getTableDataAfterIn(code) {
                ...
                api.allotin.queryByInCode(option, (res) => {
                    if(res.data.code === 'success') {
                        ...
                        _this.isPrint = true
                        _this.tempData.totalMoney = _this.tempData.totalMoney ? (_this.tempData.totalMoney - 0).toFixed(2) : _this.tempData.totalMoney
        ...
        watch: {
            ...
            //         const _this = this
            //         _this.instorageTotalMoney = 0
            //         value.transferSaleOutSkuDetailInfoVoList.forEach(element => {
            //             _this.instorageTotalMoney += (element.inNums - 0)*element.salePrice
            //         })
            //     },
            //     deep: true
            // }
