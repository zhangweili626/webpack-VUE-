<template>
    <div class="back-card">
        <b-card :header="`退卡单号：${cardRefundOrderNo}&nbsp;&nbsp;审批状态：${oprStatus}`">
            <div class="row">
                <div class="col-md-12">
                    <div class="pull-left">
                        <b-button size="sm" variant="default" @click="cancel" v-show="true">取消</b-button>
                        <b-button size="sm" variant="success" @click="saveData" v-show="btnStatus.saveData && !onlySee">保存</b-button>
                        <b-button size="sm" variant="warning" @click="editOrder" v-show="btnStatus.editOrder && !onlySee">编辑</b-button>
                        <b-button size="sm" variant="danger" @click="repealOrder" v-show="btnStatus.repealOrder && !onlySee">撤销</b-button>
                        <b-button size="sm" variant="danger" @click="InvalidOrder" v-show="btnStatus.InvalidOrder && !onlySee">作废</b-button>
                        <b-button size="sm" variant="primary" @click="submitOrder" v-show="btnStatus.submitOrder && !onlySee">提交审批</b-button>
                    </div>
                </div>
            </div>
		   <div class="row">
        ...
        <el-tabs type="border-card">
            ...
            <div class="animated fadeIn">
                 <approval-record :orderNo='cardRefundOrderNo'></approval-record>
            </div> 
            </el-tab-pane>
        </el-tabs>
        <b-card class="mt-4" v-show="isShowPayDetail">
            <div class="row">
                <div class="col-md-12">
                    <div class="pull-left">
                        <h6>退款支付明细</h6>
                    ...
                    <div class="pull-right">
                        <b-button size="sm" variant="success" @click="savePaylist" v-show="isEditPay && !onlySee">完结</b-button>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12 mt-4  mb-2 pl-0">
                    <div class="pull-left"> 
                        <b-button size="sm" variant="success" @click="myModalRefShow" v-show="isEditPay && !onlySee">新增</b-button>
                        <b-button size="sm" variant="danger" @click="delPayList" v-show="isEditPay && !onlySee">删除</b-button>
                    </div>
                    <div class="pull-right">
...
export default {
    ...
    methods: {
        ...
        _initData(val) {
            ...
            if(  routeQuery.cardNo == undefined || routeQuery.cardNo == '') {
				this.$router.push({name: 'back-card'}); 
            }
            this.cardNo =  routeQuery.cardNo;
            if((routeQuery.id == undefined || routeQuery.id == '') && val == undefined) {
                this.createOrdCode().then(res => {
                    this.cardRefundOrderNo = res;
                    this.orderStatus = '-2';
                    this.setStatus(this.orderStatus);
                    api.returnCard.getCardRefundOrderInfoByCardCode({
                        cardNo: routeQuery.cardNo
                    },(msg)=>{
                        if(msg.data.message == 'success'){
                            let dataObj = msg.data.obj                        
                            if(dataObj != null) {
                                dataObj.cardRefundOrderNo = res;
                ...
                this.OrderId = val ? val : routeQuery.id;
                api.returnCard.getCardRefundOrderInfoByCardCode({
                    id: this.OrderId
                },(msg)=>{
                    if(msg.data.message == 'success'){
