<template>
    <div>
        <b-card>
            <div>
                申请单号: {{ addInfo.cardRefundOrderNo }}
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="pull-right">
                        <b-button v-if="isShowSubmit&&!justShowDetail" size="sm" variant="success" @click="submitApprove">提交审批</b-button>
                        <b-button v-if="isShowRepeal&&!justShowDetail" size="sm" variant="primary" @click="cancelSubmit">撤销审批</b-button>
                        <b-button v-if="isShowDelete&&!justShowDetail" size="sm" variant="danger" @click="deleteSubmit">作废</b-button>
                        <b-button v-if="isShowSave&&!justShowDetail" size="sm" variant="success" @click="saveCardInfo">保存</b-button>
                        <b-button size="sm" variant="" @click="cancelEvent">取消</b-button>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody class="">
                        <tr class="">
                            ...
                            <td class="">
                                <b-form-input type="number" :state="mustState.refundTotalAmount" :readonly="isShowRepeal || isShowRefundDetail || justShowDetail" v-model="addInfo.refundTotalAmount" :max="accountLimitTotal" min="0"/>
                            </td>
                            <td class="">
                                <b-form-input type="number" :state="mustState.refundFeeCount" :readonly="isShowRepeal || isShowRefundDetail || justShowDetail" v-model="addInfo.refundFeeCount" :max="addInfo.refundTotalAmount" min="0"/>
                            </td>
        ...
        <b-card no-body>
            <div class="specialTabs">
                <b-tabs card>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-lg-4">
                                <b-form-fieldset horizontal label="实退人手机号*" label-text-align="right" :label-cols="4">
                                    <b-form-input v-model="addInfo.refundMobilePhone" :readonly="isShowRepeal || isShowRefundDetail || justShowDetail" @blur.native="getCustomerInfo" :state="mustState.refundMobilePhone"/>
                                </b-form-fieldset>
                            </div>
                            <div class="col-md-4 col-lg-4">
                            ...
                            <div class="col-md-4 col-lg-4">
                                <b-form-fieldset horizontal label="实退人*" label-text-align="right" :label-cols="4">
                                    <b-form-input v-model="addInfo.refundCustomName" :readonly="customNameDisabled || isShowRepeal || isShowRefundDetail || justShowDetail" :state="mustState.refundCustomName"/>
                                </b-form-fieldset>
                            </div>
                            <div class="col-md-4 col-lg-4">
                            ...
                            <div class="col-md-4 col-lg-4">
                                <b-form-fieldset horizontal label="固话号" label-text-align="right" :label-cols="4">
                                    <b-form-input v-model="addInfo.refundCustomPhone" :readonly="isShowRepeal || isShowRefundDetail || justShowDetail"/>
                                </b-form-fieldset>
                            </div>
                            <div class="col-md-4 col-lg-4">
                            ...
                            <div class="col-md-8 col-lg-8">
                                <b-form-fieldset horizontal label="退款说明*" label-text-align="right" :label-cols="2">
                                    <b-form-textarea id="textarea1"
                                        ...
                                        placeholder="必填"
                                        :readonly="isShowRepeal || isShowRefundDetail || justShowDetail"
                    ...
                    <b-tab title="审批履历">
                        <approve :orderNo="addInfo.cardRefundOrderNo"></approve>
                    </b-tab>
                </b-tabs>
            </div>
        </b-card>
        <b-card header="退款明细" v-if="isShowRefundDetail">
            <div class="row" v-if="isShowRefundDetail && !justShowDetail">
                <div class="col-md-6" v-if="!orderHasfinished">
                    <div class="pull-left">
                        <b-button size="sm" variant="success" @click="showAddModal">新增</b-button>
                        <b-button size="sm" variant="danger" @click="deleteItem">删除</b-button>
                    </div>
                </div>
                <div class="col-md-6" v-if="!orderHasfinished">
                    <div class="pull-right">
                        <b-button :disabled="!isShowfinished" size="sm" variant="primary" @click="finished">完结</b-button>
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <el-dialog ref="dialog" 
            ...
            size="small"  @open="reset"  :visible.sync="dialogInvoive" :modal-append-to-body="false">
            <div class="row">
                <div class="col-md-12 col-lg-12">
                    <b-form-fieldset horizontal label="支付方式" label-text-align="right" :label-cols="4">
                        <b-form-select :state="addDetailState.paymentTypeCode" v-model="addDetail.paymentTypeCode" :options="paymentStyleList"/>
                    </b-form-fieldset>
                </div>
                <div class="col-md-12 col-lg-12">
                    <b-form-fieldset horizontal label="支付金额" label-text-align="right" :label-cols="4">
                        <b-form-input type="number" :state="addDetailState.paymentAmount" v-model="addDetail.paymentAmount"/>
                    </b-form-fieldset>
                </div>
                <div class="col-md-12 col-lg-12">
                    <b-form-fieldset horizontal label="交易流水号" label-text-align="right" :label-cols="4">
                        <b-form-input :state="addDetailState.paymentCode" v-model="addDetail.paymentCode"/>
                    </b-form-fieldset>
                </div>
                <div class="col-md-12 col-lg-12">
                    <b-form-fieldset horizontal label="操作时间" label-text-align="right" :label-cols="4">
                        <!-- <b-form-input v-model="addDetail.paymentCode"/> -->
                        <el-date-picker
                            :class="{'inp-invalid' : addDetailState.paymentTime}"
                            v-model="operateTime"
                            type="datetime"
                            placeholder="选择日期时间">
                        </el-date-picker>
            ...
            <div slot="footer" class="dialog-footer dialog-footer-cla">         				
                ...
                <b-btn size="sm" class="float-right" variant="success" @click="handleOk">保存</b-btn>
            </div>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            getValueCardInfo(arg) {
                ...
                api.returnCard.getCardRefundOrderInfoByCardCode(option, res => {
                    if(res.data.code === 'success') {
