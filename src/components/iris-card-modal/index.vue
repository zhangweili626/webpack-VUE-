    ...
    <div class="card-select-modal">
        <b-modal id="modalPrevent"
                ...
                <div class="row card-select-layout">
                    <div id="card-select-need-id" class="col-md-4 col-lg-4 card-select-list">
                        <div class="card-header-title">
                            卡信息
                        </div>
                        <ul>
                            <li v-for="(item, index) in mainlist" :key="index">
                                <a href="javascript:;" @click="showThisCardDetail(item)" :class="{'current-choose-item-color' : item.currentChoose}">{{ item.cardName + '【' + item.cardNo + '】' }}</a>
                            </li>
                        </ul>
                    </div>
                        ...
                        <div class="card-select-right-detail-container">
                            ...
                            <div class="table-scrollable card-select-right-detail-table" v-if="isShowChoose">
                                <b-table striped hover bordered show-empty :items="detaillist" :fields="fieldsChoose">
                                    <template slot="index" slot-scope="data">
                                        <input type="radio" :disabled="data.item.disabled" name="currentChoose" :value="data.item" v-model="currentChoose"/>
                                    </template>
                                    <template slot="detailName" slot-scope="data">
                                        {{ data.item.detailName + '【' + data.item.detailCode + '】' }}
                                    </template>
        ...
        methods: {
                ...
                api.cardManage.queryusableCardInfos(option, res => {
                    if(res.data.code === 'success') {
                        ...
                        if(this.mainlist.length > 0) {
                            this.mainlist[0].currentChoose = true
                            let option = {
                                    ...
                                    if(this.isShowChoose) {
                                        ...
                                        api.cardManage.queryOrderServiceCardLockNum(params, res => {
                                            if(res.data.code === 'success') {
                                                ...
                                                this.detaillist.forEach(element => {
                                                    if(this.acceptObj.detailCodes) {
                                                        if(element.availableNums > 0 && element.detailCode === this.acceptObj.detailCodes[0]) {
            ...
            showThisCardDetail(value) {
                for(let i = 0; i< this.mainlist.length; i++) {
                    if(this.mainlist[i].cardNo === value.cardNo) {
                        this.mainlist[i].currentChoose = true
                    }else {
                        this.mainlist[i].currentChoose = false
                    }
                }
                let option = {
                    cardNo: value.cardNo
                    ...
                    if(res.data.code === 'success') {
                        ...
                        api.cardManage.queryOrderServiceCardLockNum(params, res => {
                            if(res.data.code === 'success') {
                                ...
                                this.detaillist.forEach(element => {
                                    if(this.acceptObj.detailCodes) {
                                        if(element.detailCode === this.acceptObj.detailCodes[0]) {
            ...
            useNow() {
                ...
                this.$emit('use', this.currentChoose)
                this.modalHide()
            },
            getFirstData(params) {
                // let option = {
                ...
                api.cardManage.queryusableCardInfos(params, res => {
                    if(res.data.code === 'success') {
                        this.mainlist = res.data.obj
                        if(this.mainlist.length === 0) {
                            return
...
<style>
    ...
    a.current-choose-item-color {
        color: rgb(74, 118, 199) !important;
    }
    ...
    #card-select-need-id.card-select-list ul>li>a {
        color: #999;
    }
</style>
